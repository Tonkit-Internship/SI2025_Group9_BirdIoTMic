...
# โหลดเวลาเก่าจาก SD card
last_timestamp = 0
try:
    with open("/sd/last_time.txt") as f:
        last_timestamp = int(f.read().strip())
        print(f"⏱ โหลดเวลาเก่าจาก SD: {last_timestamp}")
except:
    print("⚠️ ไม่มี last_time.txt หรืออ่านไม่ได้")
...
# -- backup time --
current_timestamp = int(time.time() + time_offset)
try:
    with open("/sd/last_time.txt", "w") as f:
        f.write(str(current_timestamp))
    log(f"🕒 บันทึกเวลา {current_timestamp} → /sd/last_time.txt")
except Exception as e:
    log(f"❌ บันทึกเวลาไม่สำเร็จ: {e}")
...
os.umount("/sd")
power_off_for(i2c, 150-int(elapsed))
